...
import { AgentDriversTable } from "@/components/admin/agents/AgentOnboardingTables";
import { useAgentDetailQuery } from "@/hooks/queries/use-agent-detail-query";
import { getAgentDrivers } from "@/lib/agent-onboarding";

export default function AdminAgentDriversPage() {
  ...
  return (
    <AppShell title={`Drivers • ${agent?.name || "—"}`} wideContent>
      <PageContainer>
        ...
        <SectionCard
          ...
          description={`Complete list of drivers added by ${agent?.name || "this agent"}.`}
        >
          {isLoading ? (
