  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="left-0 right-0 top-0 flex h-[100dvh] w-screen max-h-[100dvh] max-w-none translate-x-0 translate-y-0 flex-col overflow-hidden rounded-none p-0 data-[state=open]:slide-in-from-right-full data-[state=closed]:slide-out-to-right-full md:left-64 md:w-[calc(100vw-16rem)]">
        <DialogHeader>
          <div className="space-y-1.5 border-b border-border px-4 pb-4 pt-5 sm:px-6">
            <DialogTitle>{title}</DialogTitle>
            <DialogDescription>{description}</DialogDescription>
          </div>
        </DialogHeader>
        <div className="max-h-[calc(100dvh-10.5rem)] overflow-y-auto px-4 py-5 sm:px-6 scrollbar-thin scrollbar-brand">
          <div className="space-y-5">{children}</div>
        </div>
        <DialogFooter className="mt-auto border-t border-border bg-muted/20 px-4 py-3.5 sm:px-6">
