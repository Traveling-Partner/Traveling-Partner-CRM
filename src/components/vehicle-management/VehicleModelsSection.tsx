...
import { useVehicleModelsQuery } from "@/hooks/queries";
...
export function VehicleModelsSection() {
  ...
  const vehicleModels = (modelsQuery.data?.content ?? []) as VehicleEntity[];
  ...
  const openEdit = (model: VehicleEntity) => {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteVehicleModel(deleteId, { token });
