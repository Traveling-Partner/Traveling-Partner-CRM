...
| ID | Case | Expected |
|----|------|----------|
| DQ1 | Driver + partner list for queue | Uses list APIs only — badges from embedded `cnicStatus` / `licenseStatus` / `vehicleDocStatus` (null → PENDING). No per-id `/users/documents/:id` fan-out on queue load. Preview/detail still call `/users/documents/:id` once. |
